...
class Board {
   public:
    ...
    std::variant<double, Movement>     MinimaxSearch(const std::optional<Movement>& playing_move,
                                                     int*                           nodesvisited,
                                                     double                         alpha,
                                                     double                         beta,
                                                     int                            depth = 5,
                                                     Color maximizingColor = Color::kWhite,
                                                     bool  randomize       = true);
    std::variant<double, Movement>     PickOrEvaluate(const std::optional<Movement>& playing_move,
                                                      int*                           nodesvisited,
                                                      double&                        alpha,
                                                      double&                        beta,
                                                      int                            depth = 5,
                                                      Color maximizingColor = Color::kWhite,
                                                      bool  randomize       = true);
    std::shared_ptr<PieceGroup>&       GetPieces() { return pieces_; }
    const std::shared_ptr<PieceGroup>& GetPieces() const { return pieces_; }
    ...
    const Player& GetCurrentPlayer() const { return GetPlayer(currentTurn_); }
    ...
    std::pair<Movement, bool>
         LookForCheckMateMoveDfs(int                           depth,
