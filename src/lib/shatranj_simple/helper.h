...
struct DeferedCall {
    ...
    DeferedCall(FunctionType function) :
        function_(function) {}

    ~DeferedCall() { function_(); }
...
template<typename T>
auto RunWithTiming(const std::string&         header,
                   const T&                   ftocall,
    ...
    std::cout << header << " took: "
              << std::chrono::duration_cast<std::chrono::microseconds>(mduration).count() << " us"
              << std::endl;
    return ret;
}

template<typename T>
auto RunWithTimingNoRet(const std::string& header, const T& ftocall) {
    auto start = std::chrono::high_resolution_clock::now();
    ftocall();
