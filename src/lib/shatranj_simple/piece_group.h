...
class PieceGroup {
   public:
    ...
    inline static uint8_t  Coord2to1(Position pos) { return pos.Getx() + 8 * pos.Gety(); }
    inline static Position Coord1to2(uint8_t coord) { return Position(coord % 8, coord / 8); }

    size_t Size() const { return counter_; }

    size_t GetBlackCount() const { return black_count_; }

    size_t GetWhiteCount() const { return white_count_; }
    ...
    Position GetWhiteShahPos() const { return whiteShahPos_; }

    Position              GetBlackShahPos() const { return blackShahPos_; }
