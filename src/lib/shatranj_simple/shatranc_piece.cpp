...
std::array<std::array<std::array<PreComputeTableInternalContainer,
...
Piece::Piece(ChessPieceEnum pieceType, Position pos, const Color color, bool moved) :
    PiecePrimitive(pieceType, color, moved),
    pos_(pos) {
    if (!pos.IsValid())
...
Shah::Shah(Position pos, Color color) :
    Piece(ChessPieceEnum::kShah, pos, color, false) {}
Rook::Rook(Position pos, Color color) :
    Piece(ChessPieceEnum::kRook, pos, color, false) {}

Piyade::Piyade(Position pos, Color color) :
    Piece(color == Color::kWhite ? ChessPieceEnum::kPiyadeWhite : ChessPieceEnum::kPiyadeBlack,
          pos,
          color,
          false) {}

Horse::Horse(Position pos, Color color) :
    Piece(ChessPieceEnum::kHorse, pos, color, false) {}

Fil::Fil(Position pos, Color color) :
    Piece(ChessPieceEnum::kFil, pos, color, false) {}

Vizier::Vizier(Position pos, Color color) :
    Piece(ChessPieceEnum::kVizier, pos, color, false) {}
