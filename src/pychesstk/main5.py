...
class ChessGUI:
    def __init__(self, master):
        self.master = master
        master.title("Chaturanga Chess GUI (Resizable with Editor & FEN Support)")

        # Default board size; will update as the window is resized.
        self.board_size = 600  
        self.sq_count = 8  # 8x8 board
        self.square_size = self.board_size // self.sq_count

        # Main container frame for board and control panel.
        ...
        # Create a control panel below the board.
        ...
        # Configure grid weights so that the board and control panel expand.
        ...
        # Optionally, for an empty starting board, uncomment:
        # self.board.clear_board()

        # To track the selected square (for both game and editor "move" actions).
        self.selected_square = None

        # Load piece images (scaled to the current square size).
        self.load_images()
    ...
    def on_canvas_resize(self, event):
        """Resize the board (keeping it square) and reload images on canvas size changes."""
    ...
    def load_images(self):
        """Load/reload piece images scaled to the current square size."""
        self.piece_images = {}
        pieces = ['P', 'R', 'N', 'B', 'Q', 'K']
    ...
    def load_fen(self):
        """Load a board position from the FEN string in the entry field."""
    ...
    def legal_moves_for_piece(self, square):
        """
        Generate a list of destination squares for a piece on the given square,
        according to Chaturanga rules.
        """
        piece = self.board.piece_at(square)
        if piece is None:
            return []
        ...
        if piece.piece_type == chess.PAWN:
            ...
            # In Chaturanga, the "elephant" moves exactly two squares diagonally, jumping over the intermediate square.
            ...
            # In Chaturanga, the "mantri" moves one square diagonally.
            for df, dr in [(1, 1), (1, -1), (-1, 1), (-1, -1)]:
                new_file = file + df
                new_rank = rank + dr
                if 0 <= new_file < 8 and 0 <= new_rank < 8:
    ...
    def on_click(self, event):
        ...
        if self.editor_mode.get():
            ...
            if action == "remove":
                ...
                if self.board.piece_at(clicked_square) is None:
                    ...
                    try:
                        # chess.Piece.from_symbol expects just the letter;
                        # here we set color based on the prefix.
        ...
        if self.selected_square is None:
            ...
            # Use our custom legal moves generator.
            legal = self.legal_moves_for_piece(self.selected_square)
            if clicked_square in legal:
                piece = self.board.piece_at(self.selected_square)
                # (For this example we simply remove the piece from its original square and set it on the target.)
                self.board.remove_piece_at(self.selected_square)
                self.board.set_piece_at(clicked_square, piece)
                self.draw_board()
                self.draw_pieces()
                # Switch turn (this example does not enforce check or other rules).
