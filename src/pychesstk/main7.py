...
class ChessGUI:
    def __init__(self, master):
        ...
        # For Chaturanga you might wish to use a custom starting position.
    ...
    def export_pgn(self):
        """Generate a simple PGN-like text from the move history and display it in a popup."""
        ...
        for i in range(0, len(moves), 2):
            ...
            if black_move:
                pgn_lines.append(f"{move_number}. {white_move} {black_move}")
            else:
                pgn_lines.append(f"{move_number}. {white_move}")
            move_number += 1
        pgn_text = "\n".join(pgn_lines)
